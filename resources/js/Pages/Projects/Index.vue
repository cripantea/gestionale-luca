...
const getStatusBadge = (status) => {
    const badges = {
        ...
        'completato': 'bg-blue-100 text-blue-800 border-blue-200',
...
// Lista tipi di progetto unici
const uniqueProjectTypes = computed(() => {
    const types = props.projects.map(project => ({
...
const filteredProjects = computed(() => {
    return props.projects.filter(project => {
        ...
        if (searchQuery.value) {
            ...
            const matchesContact = project.contact.name.toLowerCase().includes(query);
        ...
        // Filtro tipo
        if (typeFilter.value !== 'all' && project.project_type.id !== parseInt(typeFilter.value)) {
            return false;
        }
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <Link :href="route('projects.create')" class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all">
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="mb-6 overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                    <div class="p-6">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                            ...
                            <!-- Type Filter -->
                            <div>
                                <label for="type-filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tipo</label>
                                <select
                                    v-model="typeFilter"
                                    id="type-filter"
                                    class="block w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 sm:text-sm"
                                >
                                    ...
                                    <option v-for="type in uniqueProjectTypes" :key="type.id" :value="type.id">
                                        {{ type.name }}
                                    </option>
                                </select>
                            </div>
                        ...
                        <div v-if="searchQuery || statusFilter !== 'all' || typeFilter !== 'all'" class="mt-4 flex items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-4">
                            <div class="text-sm text-gray-600 dark:text-gray-400">
                                Mostrando {{ filteredProjects.length }} di {{ projects.length }} progetti
                            </div>
                            <button
                                @click="resetFilters"
                ...
                <!-- Projects Table -->
                <div v-if="filteredProjects.length > 0" class="overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
                                <tr v-for="project in filteredProjects" :key="project.id" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <span class="inline-flex rounded-full border border-blue-200 bg-blue-100 px-2 text-xs font-semibold leading-5 text-blue-800">
                                            {{ project.project_type.name }}
                                        </span>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4">
                                        <span :class="getStatusBadge(project.status)" class="inline-flex rounded-full border px-2 text-xs font-semibold leading-5">
                                            {{ getStatusLabel(project.status) }}
                                        </span>
                                    </td>
                                    <td class="whitespace-nowrap px-6 py-4 text-sm text-gray-500">
                                        <span class="inline-flex rounded-full border border-gray-200 bg-gray-100 px-2 text-xs font-semibold leading-5 text-gray-800 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300">
                ...
                <div v-else class="overflow-hidden bg-white shadow-sm sm:rounded-lg dark:bg-gray-800">
                    <div class="p-12 text-center">
                        ...
                        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                            {{ projects.length === 0 ? 'Nessun progetto' : 'Nessun progetto trovato' }}
                        </h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                            {{ projects.length === 0 ? 'Inizia creando un nuovo progetto.' : 'Prova a modificare i filtri di ricerca.' }}
                        </p>
                        <div class="mt-6">
                            <Link v-if="projects.length === 0" :href="route('projects.create')" class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                                Crea Progetto
                            </Link>
